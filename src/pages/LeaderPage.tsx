...
import { Plus, Trash2, Copy, Share2, Check, Footprints, ArrowRight } from 'lucide-react';
...
const LeaderPage: React.FC = () => {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-steel -mt-2 mb-1">設計追蹤符號順序，分享給隊員沿途追尋</p>

      {/* Create New Button */}
      <button
        onClick={() => nav('/leader/create')}
      ...
      >
        ...
        <div className="flex-1 text-left">
          ...
          <p className="text-steel text-xs">自訂符號順序，生成分享代碼</p>
        </div>
            ...
            >
              ...
              <div className="p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-steel">
                    {trail.steps.length} 個符號 · {trail.id}
                  </p>
                </div>
              </div>
              ...
              <div className="px-3 pb-2 flex items-center gap-1 overflow-x-auto">
                {trail.steps.map((step, j) => {
                  const sign = getSignById(step.signId);
                  return (
                    <React.Fragment key={j}>
                      {j > 0 && <ArrowRight size={12} className="text-steel shrink-0" />}
                      <div className="shrink-0 flex flex-col items-center">
                        <SignSVG signId={step.signId} size={32} glow={false} />
                        <span className="text-[8px] text-steel mt-0.5">{sign?.nameZh}</span>
                      </div>
                    </React.Fragment>
                  );
                })}
              ...
              <div className="flex border-t border-cyan/5">
                <button
                  onClick={() => handleCopy(trail)}
                  className={`flex-1 py-2.5 flex items-center justify-center gap-1.5 text-xs font-heading font-medium transition-colors ${
                    copiedId === trail.id
                      ? 'text-green bg-green/5'
                      : 'text-cyan hover:bg-cyan/5'
                  }`}
                >
                  {copiedId === trail.id ? <Check size={14} /> : <Share2 size={14} />}
                  {copiedId === trail.id ? '已複製' : '複製代碼'}
                </button>
                <button
                  onClick={() => setConfirmDelete(trail.id)}
      ...
      <div className="bg-navy-800/40 rounded-2xl p-4 border border-cyan/5 space-y-1.5">
        ...
        <ol className="text-[11px] text-steel space-y-0.5 list-decimal list-inside">
          <li>建立路線，輸入名稱及符號順序</li>
            <li>按沿途順序加入追蹤符號（方向、警告、資訊）</li>
          <li>複製路線代碼，發送給隊員</li>
          <li>隊員輸入代碼後，可沿途辨識符號前進</li>
        ...
        {confirmDelete && (
          ...
          >
            ...
            >
              ...
              <p className="text-steel text-sm mb-4">此操作無法還原，確定要刪除這條路線嗎？</p>
              <div className="flex gap-2">
                <button
                  onClick={() => setConfirmDelete(null)}
                  className="flex-1 py-2.5 bg-navy-800 text-steel rounded-xl font-heading font-semibold text-sm"
                >
                  取消
                </button>
                <button
                  onClick={() => handleDelete(confirmDelete)}
                  className="flex-1 py-2.5 bg-red/10 text-red border border-red/20 rounded-xl font-heading font-semibold text-sm"
                >
                  確認刪除
                </button>
              </div>
